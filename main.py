...
# Video file paths
HAPPY_VIDEO_PATH = "happy.mp4"
SAD_VIDEO_PATH = "sad.mp4"

# Define the duration (in milliseconds) for which the video will be paused after it finishes
VIDEO_PAUSE_DURATION_MS = 2000
...
class RealTimeEmotionDetector:
    ...
    def play_video(self, video_path: str):
        self.video_player.open(video_path)
    ...
    def execute(self, wait_key_delay=33, quit_key='q', frame_period_s=0.75):
        ...
        while cv2.waitKey(delay=wait_key_delay) != ord(quit_key):
            ...
            for lbl, rectangle, lm_points in zip(predicted_labels, rectangles, landmark_points_list):
                ...
                if lbl == 'happy':
                    self.play_video(HAPPY_VIDEO_PATH)
                    cv2.waitKey(VIDEO_PAUSE_DURATION_MS)
                elif lbl == 'sad':
                    self.play_video(SAD_VIDEO_PATH)
                    cv2.waitKey(VIDEO_PAUSE_DURATION_MS)
